...
func main() {
	http.Handle("/css/", http.StripPrefix("/css/", http.FileServer(http.Dir("./files/css")))) // доступ к стилям

	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		http.ServeFile(w, r, "files/auth.html")
	...
	err := http.ListenAndServe(":8085", nil) // задаём слушать порт
	if err != nil {
...
func ErrorHandling(e error, path string, w http.ResponseWriter) {
	tmplt := template.Must(template.ParseFiles("files/error.html"))
	data := ErrorPageData{
		Message:     e.Error(),
		RedirectURL: path,
	}
	_ = tmplt.Execute(w, data) // отправляем данные на клиентскую сторону
}
...
func FTPAuth(path string, w http.ResponseWriter, r *http.Request) *ftp.ServerConn {
	...
	log.Println(identity.Host, identity.Port)
	if err != nil {
		ErrorHandling(err, path, w)
		return nil
	}
	...
	log.Println(identity.Login, identity.Password)
	if err != nil {
		ErrorHandling(err, path, w)
		return nil
	}
...
func ClientRouterHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Println("_____________------------", path, "----", r.URL.Path)
	n := len(path)
	if n > 1 && path[n-1] == '/' {
		path = path[:n-1]
	}
	...
	tmplt := template.Must(template.ParseFiles("files/index.html"))
...
func UploadRouterHandler(w http.ResponseWriter, r *http.Request) {
	...
	_ = r.ParseMultipartForm(maxMB << 20) // анализ аргументов
...
func DownloadRouterHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = io.Copy(w, reader) // отдать клиенту читающий поток без загрузки файла в память
}
